...
精选动作目录实施后，优先在 Web 和 AVD 的 360px、412px、横竖屏检查动作库、详情和共享选择器：分类中没有“复合”，器械标签与筛选可读，四种记录类型按“负重 + 次数 / 距离或时长 / 仅次数 / 保持时间”显示，长 description 的空行和逐步换行可见且不横向溢出，来源信息不出现在任何页面。该走查不得加载图片、GIF、Media URL 或非中文 instructions。
...
2. 选择动作，按该动作唯一记录类型填写负重与次数、距离和/或时长、仅次数或保持时间，并记录休息时间；完成每组后会立即写入本地数据。
