...
- schemaVersion 为 5 的 manifest。

### 2.1 开发期数据重置

当前项目尚未发布内部测试版，也没有真实用户数据。schema v5 已直接接入记录方式、重量/计数口径、单一重量 PR 与当前默认目录；不为 v4 及更早开发快照、旧默认动作或废弃字段编写 migration、重定向或兼容读取逻辑。应用读到任何非 v5 快照时会明确提示“不兼容开发快照”并停止加载，不迁移、不静默丢弃，也不会自动清理设备或远端数据。开发者确认目标仅为隔离测试环境后，才按下列步骤人工清空并重新初始化；默认动作目录的具体规则见《默认动作库引入实施方案》，`docs/data_import/candidates.md` 是候选权威。
...
记录方式改造后，优先在 Web 和 AVD 的 360px、412px、横竖屏检查动作库、详情和共享选择器：六种记录方式使用用户文案，重量口径、计数口径与成绩方向只在适用时显示；普通动作的“重量/次数”等字段及每侧动作的“每侧次数/每侧距离/每侧保持时间或用时”均可清空重输；农夫行走的“每手重量、距离、用时”三字段自适应换行且无横向溢出。未填写项不显示伪 0，来源信息不出现在任何页面。该走查不得加载图片、GIF、Media URL 或非中文 instructions。
