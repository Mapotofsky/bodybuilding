...
| exerciseRef | kind=existing 时仅 exerciseId；kind=unknown 时仅 name、category、四项记录配置、description。 |
...
3. 创建候选自定义动作时，名称、分类、四项记录配置、description 复用现有 exercise service 的校验；真实 custom-ex ID 只在批准导入时由 repository 分配。
