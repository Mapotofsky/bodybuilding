...
当前已实现离线训练、计划/模板、训练记录月历、动作库/动作详情、本地头像资料、主题选择基础 token 和 WebDAV 手动同步。HomePage 可展示今日计划和补训建议；CalendarPage 当前仅显示实际训练记录，不渲染计划条目。

以下均为规划中，不能按当前功能描述：全页面语义色彩迁移完成、AI provider/API key 配置、AI 问答/训练分析、联网 agent 和 AI 计划候选导入。
...
- [P4 主题系统与 UI 导航](详细设计文档_P4_主题系统与UI导航.md)：主题选择、基础 token 与五项底部导航已进入实现；全页面语义色彩迁移仍在推进。
...
- 当前主题选择已可用，但全页面语义色彩迁移尚未完成；当前没有 AI 配置、AI 问答、联网检索或 AI 计划导入。AI 未配置时，现有离线训练、模板、动作库/动作详情和 WebDAV 能力不受影响。
