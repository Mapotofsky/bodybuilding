...
- 兼容基线后的持久化字段和内置目录演进必须同步更新 schema、显式 migration、默认数据、fixture、字段契约和测试；内置目录按稳定 ID 融合受控字段，保留自定义动作、历史 WorkoutDoc、tombstone、未知记录和未展示字段。
...
兼容基线后的默认数据或字段变更至少覆盖：新快照与当前目录精确一致、非空旧快照增量融合、旧快照迁移、训练历史与嵌套 ID 稳定、自定义/未知记录保留、引用无悬空、未展示字段保留、派生数据重建和 WebDAV 序列化。只允许对用户明确指定的隔离测试数据验证清理流程。
